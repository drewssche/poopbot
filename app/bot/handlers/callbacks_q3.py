...
from app.services.q2_q3_service import (
    q2_choice_from_bristol,
    render_q2_private_text,
    render_q3_private_text,
    render_q3_text,
    should_show_q2_q3_button,
)
...
async def q3_callbacks(cb: CallbackQuery) -> None:
    ...
    with db_session(_session_factory) as db:
        ...
        if (not is_private_chat) and q3_msg_id and cb.message.message_id != q3_msg_id:
            await cb.answer("\u041d\u0435\u0430\u043a\u0442\u0443\u0430\u043b\u044c\u043d\u043e", show_alert=False)
            return
        ...
        is_back_q2 = cb.data.startswith("q3:back_q2:")
        is_skip = cb.data.startswith("q3:skip:")
        if is_back_q2 or is_skip:
            try:
        ...
        if is_private_chat:
            try:
                if is_back_q2:
                    evt2 = events_by_n.get(selected_n)
                    active_q2 = q2_choice_from_bristol(evt2.bristol if evt2 else None)
                    await cb.message.edit_text(
                        render_q2_private_text(db, sess.session_id, user.id, selected_n),
                        reply_markup=q2_keyboard(
                            selected_choice=active_q2,
                            private_flow=True,
                            event_n=selected_n,
                        ),
                    )
                ...
                if "message is not modified" not in str(e).lower():
                    logger.exception("Failed to edit private flow from Q3: %s", e)
