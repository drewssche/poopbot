...
from app.db.models import ChatMember, PoopEvent, User
from app.db.models import Session as DaySession
from app.services.recap_common import bot_first_interaction_date, phrase_toilet, year_flavor


def build_my_year_recap_cards(db: Session, chat_id: int, user_id: int, year: int) -> list[str]:
    first_interaction = bot_first_interaction_date(db)
    start = date(year, 1, 1)
    ...
    feb9 = int(day_totals.get(date(year, 2, 9), 0))
    nov19 = int(day_totals.get(date(year, 11, 19), 0))

    title, intro, outro = year_flavor(year)
...
def build_chat_year_recap_cards(db: Session, chat_id: int, year: int) -> list[str]:
    ...
    feb9_total = int(day_totals.get(date(year, 2, 9), 0))
    nov19_total = int(day_totals.get(date(year, 11, 19), 0))
