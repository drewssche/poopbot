...
class TimeSlotTests(unittest.TestCase):
    ...
    def test_get_slot_emoji(self) -> None:
        ...
        self.assertEqual(get_slot_emoji("all_day"), "🕐")
    ...
    def test_get_slot_title(self) -> None:
        ...
        self.assertEqual(get_slot_title("morning"), "Утренний просер")
        self.assertEqual(get_slot_title("afternoon"), "Дневной навальщик")
        self.assertEqual(get_slot_title("evening"), "Вечерний сливатор")
        self.assertEqual(get_slot_title("all_day"), "Круглосуточный серун")
    ...
    def test_get_dominant_slot_low_sample_returns_none(self) -> None:
        """Мало данных для титула."""
        counts = {"night": 1, "morning": 1, "afternoon": 0, "evening": 0}
        self.assertIsNone(get_dominant_slot(counts))

    def test_get_dominant_slot_single_slot_low_sample_keeps_title(self) -> None:
        """Один слот даже при малом количестве должен получать титул."""
        counts = {"night": 1, "morning": 0, "afternoon": 0, "evening": 0}
        self.assertEqual(get_dominant_slot(counts), "night")

    def test_get_dominant_slot_tie_returns_none(self) -> None:
        """Ничья между лидерами."""
        counts = {"night": 3, "morning": 3, "afternoon": 1, "evening": 0}
        self.assertIsNone(get_dominant_slot(counts))
